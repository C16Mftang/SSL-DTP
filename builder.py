...
class NetBP(nn.Module):
    ...
    def __init__(self, batch_size, step_size, device, loss_param, p=0., loss='SimCLR'):
        ...
        if loss == 'Naive':
            ...
            self.criterion = losses.BarlowTwinsLoss(loss_param['lambda'], loss_param['scale'], 
                                                    batch_size=batch_size, device=device)
...
class NetDTP(nn.Module):
    ...
    def __init__(self, batch_size, step_size1, step_size2, device, loss_param, sigma=1, lr_targ=0.5, p=0., loss='SimCLR'):
        ...
        if loss == 'Naive':
            ...
            self.global_criterion = losses.SSHingeLoss(loss_param['margin_pos'], loss_param['margin_neg'], 
                                                batch_size=batch_size, device=device)
        elif loss == 'Barlow':
            self.global_criterion = losses.BarlowTwinsLoss(loss_param['lambda'], loss_param['scale'], 
                                                    batch_size=batch_size, device=device)
        # local loss
